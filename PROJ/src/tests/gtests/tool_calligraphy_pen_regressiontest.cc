...
class ToolCalligraphyPenRegressionTest : public ::testing::Test {
  void SetUp() override {}

 protected:
  ToolCalligraphyPen callig_;
  const std::string input_file = "./resources/test-standard.png";
  const std::string file_truth = "./resources/tool_calligraphy_pen_truth.png";
};

TEST_F(ToolCalligraphyPenRegressionTest, MatchesTestToolImage) {
  ...
  // t_editor->CreateToolTruthFile(callig_.name(), input_file, file_truth);

  t_editor->StartStroke(callig_.name(), ColorData(0.5, 0.5, 0.5), 10.0, 10, 10);
